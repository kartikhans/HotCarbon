...
table_id_directory = dict(
    user=os.environ["USER_TABLE"],
    credit_card=os.environ["CREDIT_TABLE"],
...
def generate_random_credit_data(user_id):
    return [
        dict(
            ...
            last_four=random.randint(1001, 9999),
            category=random.choice(card_types),
            user_id=user_id,
            ...
            for j in range(3):
                ...
                for k in range(5):
                    transaction_data = generate_random_transaction_data(
                        credit_card_data[0]["card_id"]
                    )
                    self.push_data(
                        transaction_data, table_id_directory.get("transaction")
                    )
